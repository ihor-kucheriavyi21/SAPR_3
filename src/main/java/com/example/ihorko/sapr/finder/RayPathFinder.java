...
public class RayPathFinder extends BaseFinder {
    ...
    public char[][] findRoute(char[][] map, Element startElement, Element endElement) {
        ...
        Result result = rayPropagation(intMap, startPoint, endPoint, K, L);
        if (!result.isReached()) {
            doneMap = null;
        } else {
    ...
    private Result rayPropagation(int[][] map, Point startPoint, Point endPoint, int K, int L) {
        ...
        while (allPoints > 0) {
            ...
            while (lenQueue > 0) {
                ...
                Point[] neighborPoints = getNeighbors(thisPoint, endPoint, K, L);
    ...
    private int[][] pathRecovery(int[][] map, Point startPoint, Point endPoint, int counter, int K, int L) {
        ...
        while (counter > 0) {
            Point[] neighborPoints = getNeighbors(nextPointsQueue.pollFirst(), startPoint, K, L);
            for (Point point : neighborPoints) {
                if (point == null) {
                    ;
    ...
    private Point[] getNeighbors(Point point, Point endPoint, int K, int L) {

        Point[] neighbors = new Point[4];
        neighbors[0] = point.x <= endPoint.x ? null : point.getUpperPoint();
        neighbors[1] = point.x >= endPoint.x ? null : point.getLowerPoint();
        neighbors[2] = point.y <= endPoint.y ? null : point.getLeftPoint();
        neighbors[3] = point.y >= endPoint.y ? null : point.getRightPoint();
       /* if (point.x > endPoint.x && point.x - 1 >= 0) {
            neighbors[0] = point.getUpperPoint();
        } else
            neighbors[0] = null;
        if (point.x < endPoint.x && point.x + 1 <= K - 1)
            neighbors[1] = point.getLowerPoint();
        else
            neighbors[1] = null;
        if (point.y > endPoint.y && point.y - 1 >= 0)
            neighbors[2] = point.getLeftPoint();
        else
            neighbors[2] = null;
        if (point.y < endPoint.y && point.y + 1 <= L - 1)
            neighbors[3] = point.getRightPoint();
        else
            neighbors[3] = null;
*/
        return neighbors;
